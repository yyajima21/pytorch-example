...
from NeuroDataset import NeuroDataset
...
# Training settings.
parser = argparse.ArgumentParser(description='Neuro segmentation of cells, axons, and blood vessels.')
...
parser.add_argument('--cfgfile', type=str, default='cfg/neuro.cfg',
...
if not args.test:
    train_dataset = NeuroDataset(train_txt, im_size=args.im_size, split='train', transform=transform)
    val_dataset = NeuroDataset(train_txt, im_size=args.im_size, split='val', transform=transform)
    ...
    test_dataset = NeuroDataset(test_txt, im_size=args.im_size, split='test', transform=transform)
...
if args.test:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if (epoch % 1 == 0 and args.save_model):
            save_path = os.path.join(backup_dir, 'NeuroModel' + '_' + str(epoch) + '.pt')
